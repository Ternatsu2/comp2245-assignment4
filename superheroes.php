...
//Checks if a query was provided
if ($query) {
    $found = false;

    //Searches for the superhero in the array
    foreach ($superheroes as $superhero) {
        if (strcasecmp($superhero['alias'], $query) === 0 || strcasecmp($superhero['name'], $query) === 0) {
            //Superhero found, displays their details
    ...
    //If no superhero was found
    if (!$found) {
        echo '<p class="not-found">Superhero not found</p>';
    }
} else {
    //If no query is provided, displays the full list of superheroes
    echo "<ul>";
    foreach ($superheroes as $superhero) {
